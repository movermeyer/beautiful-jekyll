---
layout: post
title:  "Automatically fixing GitHub badges"
date:   2018-03-04 13:30:01 -0000
categories: python github badges bots
---

TODO: better title? (Building Bots to mend Broken Badges)
TODO: Update post time.

**TL;DR:** Many badges were broken since they were using the shuttered pypi.in site. I wrote a bot that could fix these and automatically submit pull requests with the changes.
...
However, within the Python community there were a bunch of badges that were broken in the package READMEs. These badges were making use of a badge service called [pypip.in](https://web.archive.org/web/20150318013508/https://pypip.in/) which apparently stopped operating some time ago (a few years?).
...
As a result, testing was very important. TODO: more on testing.

## Incremental Rollout

Even still, there is only so many error cases you can reasonably predict and properly defend against. Anytime you has a process that might cause considerable damage, you should start small and work your way up until you have some confidence that it is working. I started with batches of 1, then worked myself up to 10, and eventually 100 pull requests at a time.
...
(It also probably helped mitigate anger when my bot made a stupid mistake that would be considered malicious if it were not automated. More on that later. TODO: Actually write that.)
...
As with all your interactions should be on the Internet, you should be trying to be overly polite and friendly. Everyone comes from a different point of view, so you need to be civil. In this case, they are the maintainer of the repo; they wield ultimate authority over the codebase. If you want them to accept your pull request, you need to make sure that your messaging is friendly, humble, and deferent.
...
Once I had a script to produce the pull requests, I was nearly ready to unleash it upon the world. However, there is one step that is easy to forget to build but is very important.
...
Finally, I kept track of the time at which I created the pull requests. This is useful when debugging any issues that arise, because you get an ordered timeline of what your bot did. Having your bot output timestamped logs can also fills this need.
...
Unfortunately, I never developed a proper fix for this last bug. I added some code to check for an existing fork within the `movermeyer` namespace, and skipped the package if I had already processed a repo with a similar name. (TODO: Perhaps I could fork it, and then rename the fork?)

## Results + Feedback

Overall, the results were very positive. The bot made pull requests against xxx (TODO: real number) repositories. As of time of writing xxx (TODO: real number) of them have been merged, while only xxx (TODO: real number) were closed (ie. rejected). The ones that were rejected seem to be for repositories that are no longer being maintained, and the maintainers simply closed the pull request by default. One maintainer sent me an email and requested that I add his projects to a blacklist (which I quickly obliged).
...
This was a nice side project that taught me a lot. It allowed me to make a bunch of mistakes that I can learn from and apply the lessons to future bots.
As I briefly hinted at in the beginning of the post, this was just the first step towards a much more complicated (and useful) bot that I'm working on.

If you're interested in seeing some of the code, I have slapped together [a repo on GitHub](TODO: Actually do this) that contains the code I used for this project.
